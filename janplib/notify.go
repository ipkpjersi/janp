...
func lockFile(index int) (file string, err error) {
	...
	if err != nil {
		fmt.Println("Error with GetStorageDir in lockFile")
		return
	}
...
func lockFree(index int) (available bool, err error) {
	...
	if err != nil {
		fmt.Println("Error with lockFile in lockFree")
		return
	}
...
func Notifyf(head uint32, expire time.Duration, onInit func(*gtk.Window),
	...
	X, err := xgb.NewConn()
	if err != nil {
		fmt.Println("Error with newConn")
		return
	}
	...
	if err != nil {
		fmt.Println("Error with ScreenRects")
		return
	}
	...
	if top_right {
		...
		for ; ; lockIndex++ {
			...
			if err != nil {
				fmt.Println("Error with lockFree")
				return
			}
			if free {
